...
export class Quoter {
  ...
  protected setTrades(symbol: string, trades: Trades): void {
    if (Array.isArray(trades)) {
      ...
      this.tradesBySymbol[symbol] = trades.filter(
        (trade) =>
          now - trade.timestamp < 60 * 60 * 1000 && now >= trade.timestamp
      );
  ...
  protected setTrade(
    ...
    const currentTrade = trades.find(
      (trade) => trade.timestamp === candleTimestamp
    );

    // make 1m candle stick
    if (currentTrade) {
      currentTrade.price = price;
      currentTrade.volume = isAccumulatedVolume
        ? volume
  ...
  protected alive(): void {
    if (!this.isAlive) {
      const downtime = (
        (Date.now() - this.alivedAt - this.options.interval) /
        60 /
        1000
      ).toFixed(1);
