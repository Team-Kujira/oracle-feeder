...
export class Provider {
  ...
  public async initialize(): Promise<void> {
    ...
    this.symbols = uniq(
      concat(...this.quoters.map((quoter) => quoter.getSymbols()))
    );

    await this.tick(Date.now());

    const quoters = this.quoters
      .map((quoter) => quoter.constructor.name)
      .join(", ");
  ...
  protected collectTrades(symbol: string): Trades {
    return concat(
      ...this.quoters.map((quoter) => quoter.getTrades(symbol) || [])
    );
  }
    ...
    for (const symbol of this.symbols) {
      ...
      if (useTvwap) {
        const trades = this.collectTrades(symbol).filter(
          (trade) =>
            now - trade.timestamp < TVWAP_PERIOD && now >= trade.timestamp
        );
  ...
  private report(now: number): void {
    ...
    try {
            ...
            ...concat(
              ...this.quoters.map((quoter) =>
                concat(
                  ...quoter
                    .getSymbols()
                    .map((symbol) => `${quoter.constructor.name}\n${symbol}`)
                )
              )
            ),
          ]
      ...
      const report = {
        time: format(
          Math.floor(addMinutes(now, -1).getTime() / 60000) * 60000,
          "MM-dd HH:mm"
        ),
      };
