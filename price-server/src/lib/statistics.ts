...
export function vwap(
  array: { price: BigNumber; volume: BigNumber }[]
): BigNumber {
  if (!array || !array.length) {
    throw new Error("empty array");
  }
  ...
  const sum = array.reduce(
    (s, x) => s.plus(x.volume.multipliedBy(x.price)),
    num(0)
  );
...
export function tvwap(
  ...
  const tvwapTrades = sortedArray.map((trade) => ({
    ...
    volume: trade.volume.multipliedBy(
      weightUnit.multipliedBy(
        period.minus(now.minus(num(trade.timestamp))).plus(minimumTimeWeight)
      )
    ),
  }));
